...
class PIDCalibrate:
    ...
    def cmd_PID_CALIBRATE(self, gcmd):
        ...
        calibrate = ControlAutoTune(heater, target,heater_type)
...
class ControlAutoTune:
    def __init__(self, heater, target,heater_type):
        self.heater = heater
    ...
    def calc_pid(self, pos):
        ...
        Tu = time_diff
        #adjust calculations based on if heater is a hotend or bed/enclosure
        if self.heater_type.upper() == "BED" or  self.heater_type.upper() == "ENCLOSURE":
          pf=0.2
          df=0.33
        if self.heater_type.upper() == "HOTEND":
          pf=0.6
          df=0.125
        # Use Ziegler-Nichols method to generate PID parameters
        Kp = pf * Ku * heaters.PID_PARAM_BASE
        Ki = (Kp*2) / Tu
        Kd = Kp * Tu *df
